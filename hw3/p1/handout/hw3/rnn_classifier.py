    ...
    def forward(self, x, h_0=None):
        ...
        for s in range(seq_len):
            ...
            for l in range(self.num_layers):
                ...
                prev_t_hidden   = self.hiddens[s]
                hidden[l, :, :] = self.rnn[l].forward(x_s, prev_t_hidden[l, :, :])
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for s in range(seq_len-1, -1, -1):
            ...
            dh_prev_t = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)

            for l in range(self.num_layers-1, -1, -1):

                h_t      = self.hiddens[s+1][l, :, :]
                h_prev_t = self.hiddens[s][l, :, :]

                if l == 0:
                    h_prev_l = self.x[:, s, :]
                
                else:
                    h_prev_l = self.hiddens[s+1][l-1, :, :]

                curr_grad, prev_grad = self.rnn[l].backward(dh[l, :, :] , h_t, h_prev_l, h_prev_t)

                if l > 0:
                    dh[l-1, :, :] += curr_grad

                dh_prev_t[l, :, :] += prev_grad
